define(['angular', 'jquery', 'uiGrid'], function(angular, $) {
      ...
      ctrl.controller('testCtrl', ['$scope', '$location', '$q', '$http', '$compile', '$timeout', '$interval', function($scope, $location, $q, $http, $compile, $timeout, $interval) {
            ...
            $scope.defectSave = function() {
              alert(123);
            }

            $scope.defectClick = function(id, name) {
              var editorModal = $("#editorModal");

              if (editorModal.length > 0) {

                $scope.editorModal.id = id;
                $scope.editorModal.name = name;
                    ...
                    if (data.error) {
                      ...
                      if (data.retMap) {
                        ...
                        angular.forEach(allDefect, function(val, idx) {
                              ...
                              $(".defect").append('<span class="label ' + css + '" style="cursor: pointer;margin: 10px" defectId="' + val.id + '" ng-click="defectClick(\'' + val.id + '\',\''
                                  + val.name + '\')">' + val.name + '</span>');
                            });

                        $compile($(".defect").contents())($scope);
                      }
                    }
